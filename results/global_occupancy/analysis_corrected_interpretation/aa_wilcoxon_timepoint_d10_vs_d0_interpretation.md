...
user_directives:
  ...
  - "(readback) 'Reconciled shared content from the corrected .qmd on 2026-06-07' -> 'Adopted Olive 6 per-direction Top-hits sub-tables + raw p_value column (bare AA codes kept); corrected the headline largest-magnitude-cell-overall claim P-site K (-0.2094) -> E-site S (+0.2177, raw p 0.0571), keeping P-site K as the largest depletion; aligned Numbers site order to A/P/E; adopted the asymptotic-with-ties entry into Considered-but-not-applicable to match the aa sister.'"
---
...
- (triage, family-batched) "Spotlight any site/feature/group?" → "No spotlight"
- (readback) "Reconciled shared content from the corrected .qmd on 2026-06-07" → adopted Olive's 6 per-direction Top-hits sub-tables and raw `p_value` column (bare AA codes kept); corrected the headline largest-magnitude-cell-overall claim from P-site K (-0.2094) to E-site S (+0.2177, raw p 0.0571), keeping P-site K as the largest depletion; aligned the Numbers site order to A/P/E; adopted the asymptotic-with-ties entry under Considered but not applicable to match the aa sister `aa_wilcoxon_condition`.
...
Tables are split per site (decoding-site order A / P / E) and per direction (enriched / depleted). Two p columns are shown: `p_value` is the raw Mann-Whitney p and `p_adj` is BH-corrected within each site's 20-amino-acid family; rows are ranked by raw `p_value` ascending with `|log2_FC|` descending as the tiebreaker (the family clears no FDR threshold, so selection is by raw p). Every cell flagged `nominal-only` sits exactly at the n=4 two-sided MW floor (raw p 0.02857): its raw significance is the maximum the test can produce at this N, not evidence of a strong effect.

### Site A — enriched

| feature | effect (`log2_FC`) | raw p (`p_value`) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| E | +0.0782 | 0.0286 | 0.2857 | nominal-only |
| S | +0.0749 | 0.0286 | 0.2857 | nominal-only |
| M | +0.1381 | 0.0571 | 0.2857 |  |
| F | +0.1290 | 0.1143 | 0.3810 |  |
| R | +0.0694 | 0.1143 | 0.3810 |  |

### Site A — depleted

| feature | effect (`log2_FC`) | raw p (`p_value`) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| K | -0.0929 | 0.0571 | 0.2857 |  |
| G | -0.0472 | 0.2000 | 0.5714 |  |
| P | -0.1266 | 0.3429 | 0.7473 |  |
| Q | -0.0978 | 0.3429 | 0.7473 |  |
| W | -0.1350 | 0.4857 | 0.7473 |  |
...
| feature | effect (`log2_FC`) | raw p (`p_value`) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| C | +0.1660 | 0.0286 | 0.2857 | nominal-only |
| S | +0.0409 | 0.0571 | 0.3810 |  |
| E | +0.1775 | 0.1143 | 0.4571 |  |
| D | +0.1547 | 0.3429 | 0.9714 |  |
| N | +0.0356 | 0.4857 | 0.9714 |  |

### Site P — depleted

| feature | effect (`log2_FC`) | raw p (`p_value`) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| I | -0.0955 | 0.0286 | 0.2857 | nominal-only |
| K | -0.2094 | 0.1143 | 0.4571 |  |
| H | -0.0392 | 0.2000 | 0.6667 |  |
| G | -0.0505 | 0.4857 | 0.9714 |  |
| Q | -0.0303 | 0.4857 | 0.9714 |  |

### Site E — enriched

| feature | effect (`log2_FC`) | raw p (`p_value`) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| S | +0.2177 | 0.0571 | 0.5714 |  |
| M | +0.0968 | 0.1143 | 0.6667 |  |
| L | +0.1148 | 0.2000 | 0.6667 |  |
| C | +0.0803 | 0.2000 | 0.6667 |  |
| T | +0.0334 | 0.2000 | 0.6667 |  |

### Site E — depleted

| feature | effect (`log2_FC`) | raw p (`p_value`) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| K | -0.0864 | 0.0571 | 0.5714 |  |
| P | -0.1301 | 0.3429 | 0.8571 |  |
| G | -0.0846 | 0.4857 | 0.8831 |  |
| I | -0.0548 | 0.4857 | 0.8831 |  |
| V | -0.0476 | 0.6857 | 0.9143 |  |
