...
21 of 60 (site, aa) cells have the same OR direction in all 3 timepoints. Split by displayed mean-log2OR direction into BWM-enriched (7 rows) and BWM-depleted (14 rows) sub-tables; each is sorted by `#sig` desc, then by `|mean log2OR|` desc. `#sig` is the count (out of 3) of timepoints where this row reaches p_adj<0.05; `min p_adj` is the smallest of the 3 per-timepoint values. Flag column: `iid-amp` for any cell whose smallest per-timepoint `p_adj` falls below 1e-10; `rare-aa (<timepoint list>)` if any of the 3 timepoint rows has BWM_k<100, with the suffix listing the triggering timepoints.

#### Concordance — BWM-enriched (mean log2OR > 0)

| cell (site:aa) | mean log2OR | per-tp log2OR (d0, d5, d10) | #sig (FDR<0.05) | min p_adj | flag |
| --- | --- | --- | --- | --- | --- |
...
Counts in the concordant set: 7 enriched, 14 depleted. Of the 21, only 4 have any per-timepoint cell at p_adj<0.05; the remaining 17 are direction-consistent at sub-significance and are reported here for symmetric coverage with the direction-flip table below.

### Direction-flip cells across timepoints (equal-billing companion table)

39 of 60 (site, aa) cells show at least one sign change across the 3 timepoint values. Of these, only 1 (E:K) has the flip register at p_adj<0.05 on both opposite-sign rows — flagged `both-flip-sig`. `#sig` is the count (out of 3 timepoints) of rows reaching p_adj<0.05; `*` marks each per-timepoint value at p_adj<0.05. Sorted by `#sig` desc, then by max single-cell |log2OR| desc.

| cell (site:aa) | log2OR range | per-tp log2OR (sig at p_adj<0.05 marked `*`) | #sig (FDR<0.05) | flag |
| --- | --- | --- | --- | --- |
...
Each block is one BH-FDR family of 20 AAs at one (timepoint, site) cell. Rows with `p_adj` < 0.05 are listed; sub-tables with no qualifying rows (e.g. day_10 P) are omitted entirely.

<details>
<summary>day_0 (A / P / E)</summary>
...
*(day_10 P site has n_sig p_adj<0.05: 0/20 — table omitted entirely under the p_adj<0.05 inclusion rule.)*
