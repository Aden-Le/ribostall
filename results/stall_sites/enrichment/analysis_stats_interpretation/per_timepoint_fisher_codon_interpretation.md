...
48 of 183 (site, codon) cells hold the same OR direction across all 3 timepoints. Below the cells are split by displayed mean-log2OR direction into BWM-enriched and BWM-depleted sub-tables; each is sorted by `#sig` desc then `|mean log2OR|` desc, and capped at the top 15 cells per direction (omitted-row counts in the footer beneath each sub-table). `#sig` is the count (out of 3 timepoints) where the row reaches `p_adj<0.05`; `min p_adj` is the smallest of the 3 per-timepoint values. The `cell` column carries the `site:codon(AA)` triple — site letter (A/P/E), three-letter codon, one-letter amino acid in parentheses. The `per-tp log2OR` column lists the three per-timepoint values in chronological order (day_0, day_5, day_10); `*` marks each per-timepoint value at p_adj<0.05.

#### Concordance — BWM-enriched (mean log2OR > 0)

| cell (site:codon(AA)) | mean log2OR | per-tp log2OR (d0, d5, d10) | #sig (FDR<0.05) | min p_adj | flag |
| --- | --- | --- | --- | --- | --- |
...
133 of 183 (site, codon) cells show at least one sign change across the 3 timepoint values. Only 1 (E:AAG high-count) has the flip register at p_adj<0.05 on both opposite-sign rows — flagged `sig-flip: yes` below. `#sig` is the count (out of 3 timepoints) of rows reaching p_adj<0.05; the `per-tp log2OR` column lists the three per-timepoint values in chronological order (day_0, day_5, day_10); `*` marks each per-timepoint value that reaches the threshold. Sorted by `#sig` desc, then by max single-cell `|log2OR|` desc. Capped at the top 15 flip cells.

| cell (site:codon(AA)) | log2OR range | per-tp log2OR (d0, d5, d10) | #sig (FDR<0.05) | sig-flip | flag |
| --- | --- | --- | --- | --- | --- |
...
Each block is one BH-FDR family of 61 codons at one (timepoint, site) cell. Top hits at p_adj<0.05 with combined k_BWM+k_ctrl>=50 (all rows shown; no cap). Empty (timepoint, site) cells whose surviving row set is empty under the p_adj<0.05 cutoff are omitted entirely. Sites are listed in canonical A / P / E order within each timepoint.

<details>
<summary>day_0 (A / P / E) — 82/183 of all FDR<0.05 hits in the file</summary>

#### day_0, A site (n_sig p_adj<0.05: 27/61)
| direction | feature | effect (`odds_ratio` as log2OR) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| enriched | AAG (K) | +0.654 | 1.38e-17 | iid-amp |
| enriched | CGC (R) | +0.592 | 2.03e-03 |  |
| enriched | AGA (R) | +0.559 | 3.68e-04 |  |
| enriched | CAC (H) | +0.448 | 2.67e-02 | rare-codon |
| enriched | CTC (L) | +0.427 | 4.00e-03 |  |
| depleted | TGT (C) | -1.843 | 5.88e-04 | rare-codon |
| depleted | TTT (F) | -1.521 | 5.41e-05 | rare-codon |
| depleted | AAT (N) | -1.499 | 1.03e-09 | rare-codon |
| depleted | TAT (Y) | -1.346 | 4.13e-08 | rare-codon |
| depleted | AGT (S) | -1.340 | 2.35e-02 | rare-codon |

#### day_0, P site (n_sig p_adj<0.05: 28/61)
| direction | feature | effect (`odds_ratio` as log2OR) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| enriched | CGC (R) | +0.954 | 3.45e-06 | rare-codon |
| enriched | AAG (K) | +0.692 | 1.04e-18 | iid-amp |
| enriched | AGA (R) | +0.579 | 2.47e-04 |  |
| enriched | ATC (I) | +0.424 | 2.47e-04 |  |
| enriched | CGT (R) | +0.410 | 2.41e-03 |  |
| depleted | ACA (T) | -2.005 | 5.58e-08 | rare-codon |
| depleted | ACG (T) | -1.868 | 6.17e-04 | rare-codon |
| depleted | TGT (C) | -1.477 | 1.05e-03 | rare-codon |
| depleted | TTT (F) | -1.299 | 1.59e-04 | rare-codon |
| depleted | AGT (S) | -1.277 | 5.07e-03 | rare-codon |

#### day_0, E site (n_sig p_adj<0.05: 27/61)
| direction | feature | effect (`odds_ratio` as log2OR) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| enriched | CGC (R) | +0.923 | 1.44e-05 | rare-codon |
| enriched | AAG (K) | +0.745 | 2.03e-30 | iid-amp |
| enriched | CTC (L) | +0.539 | 1.89e-04 |  |
| enriched | AGA (R) | +0.468 | 1.66e-03 |  |
| enriched | CGT (R) | +0.465 | 3.47e-03 |  |
| depleted | ACG (T) | -2.293 | 6.32e-04 | rare-codon |
| depleted | ACA (T) | -1.555 | 2.43e-05 | rare-codon |
| depleted | CCG (P) | -1.402 | 7.20e-03 | rare-codon |
| depleted | GTA (V) | -1.402 | 7.20e-03 | rare-codon |
| depleted | TCA (S) | -1.396 | 1.44e-05 | rare-codon |

</details>

<details>
<summary>day_5 (A / P / E) — 3/183 of all FDR<0.05 hits</summary>

#### day_5, A site (n_sig p_adj<0.05: 0/61)
*(table omitted under the p_adj<0.05 cutoff; min p_adj in this cell is 0.283.)*

#### day_5, P site (n_sig p_adj<0.05: 0/61)
*(table omitted under the p_adj<0.05 cutoff; min p_adj in this cell is 6.98e-02.)*

#### day_5, E site (n_sig p_adj<0.05: 3/61)
| direction | feature | effect (`odds_ratio` as log2OR) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| enriched | CCA (P) | +0.358 | 2.52e-02 |  |
| depleted | GAG (E) | -0.300 | 5.00e-03 |  |
| depleted | AAG (K) | -0.226 | 5.00e-03 |  |

</details>

<details>
<summary>day_10 (A / P / E) — 1/183 of all FDR<0.05 hits</summary>

#### day_10, A site (n_sig p_adj<0.05: 0/61)
*(table omitted under the p_adj<0.05 cutoff; min p_adj in this cell is 9.77e-02.)*

#### day_10, P site (n_sig p_adj<0.05: 1/61)
| direction | feature | effect (`odds_ratio` as log2OR) | adjusted p (`p_adj`) | flag |
| --- | --- | --- | --- | --- |
| enriched | GTC (V) | +0.563 | 4.03e-03 |  |

#### day_10, E site (n_sig p_adj<0.05: 0/61)
*(table omitted under the p_adj<0.05 cutoff; min p_adj in this cell is 1.36e-01.)*
...
- Open questions Chumeng should resolve at synthesis time:
  ...
  - The 1 sig-flip cell counted in the report: E:AAG (high-count, day_0 +0.745 vs day_5 -0.226, both p_adj<0.05). Does the AA-resolution sister show E:K (the Lys AA cell containing AAG) flip with the same direction sequence between day_0 and day_5? If yes, the AAG-driven flip is real at AA level; if AA-level E:K does not flip (because AAA stays in the other direction or partially cancels), the flip is a codon-resolution-specific signal. (E:CCT — which has both opposite-sign rows at p_adj<0.05 in the raw data, day_0 -1.235 / day_5 +0.252 — is in rare-codon territory at day_0 BWM_k=11 and is excluded from the report's sig-flip count and from the Direction-flip top-15 display; mention as a follow-up if cross-test resolution lands on it.)
