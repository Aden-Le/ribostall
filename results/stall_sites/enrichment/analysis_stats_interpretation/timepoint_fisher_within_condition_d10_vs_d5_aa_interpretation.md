...
caveats:
  ...
  - {label: "shared-time-trajectory-A/P-Gly", proposed_by: dylan, status: confirmed, why: "Both BWM and control enrich Gly at sites A and P between d5 and d10 (BWM A:G `log2_OR`=+0.312 / P:G `log2_OR`=+0.297; control A:G `log2_OR`=+0.388 / P:G `log2_OR`=+0.429, control P:G p_adj=5.80e-08). Coherent shared-time signal — not perturbation-specific. Treat as a control-aligned time-axis feature for Chumeng, not a BWM-specific finding."}
caveats_considered: []
...
user_directives:
  ...
  - "(2026-05-28) Top hits restructured from the BWM-vs-control two-table up/down split to the three-section significance layout (both / BWM-only / control-only), each row pairing BWM and control `log2_OR` per (site, amino acid) cell with an `Effect change` (BWM minus control) column; rows grouped A/P/E then sorted by `Effect change` descending. Matches the rendered Olive `.qmd`; tables generated by `scripts/within_condition_sig_split.py`. Underlying significant cells unchanged (8 BWM + 15 control = 23). control P:G `log2_OR` corrected +0.430 -> +0.429 (raw 0.42945) in the shared-time-trajectory caveat + For-Chumeng list. Step 4 of the within_condition family three-section rollout."
  - "(readback) Reconciled shared content from the corrected .qmd on 2026-06-01 -> Top hits already mirrored the .qmd three-section layout (both / BWM-only / control-only, `effect change` column, no p_value/p_adj columns) — no table change. Number-audit (every number enumerated): all 19 table cells re-verified against the raw CSV (log2_OR, effect change = BWM - control, section placement by FDR<0.05); 1 Dylan-only CSV-column correction: `control-vs-BWM-divergent-direction` body caveat control E:R `p_adj` 0.21 -> 0.081 (raw control E,R p_adj 0.0807; 0.21 was a stray copy of the adjacent log2_OR=-0.21). Cross-file d10_vs_d0 / between_condition magnitudes in For-Chumeng flagged not-CSV-verifiable-here. _INDEX headline re-synced from odds_ratio form to log2_OR form."
---
...
- (2026-05-26) Olive `.qmd`-side raw-CSV validation of Bio-interp paragraph 5 (the "8-vs-15 power story" clause) inverted the paragraph's thesis after the per-condition d5+d10 marginals were verified balanced (BWM 18880, control 19965; ~1.06:1). Dylan-side: body caveat `large-N-Fisher-anticonservative` line `'pooled totals 6945 / 8788 / 11177 / 11935'` rewritten to `'per-timepoint per-condition totals are in the thousands (BWM: day_10=6945, day_5=11935; control: day_10=8788, day_5=11177)'` to remove the same "pooled (day_10 + day_5)" ambiguity caught in CSV 14's 2026-05-25 audit (the four numbers are per-timepoint per-condition marginals, not summed). Dylan does not echo the 8-vs-15 power-story paragraph itself, so no other Dylan-side edit.
- (2026-05-28) Top hits restructured from the BWM-vs-control two-table up/down split to the three-section significance layout (both / BWM-only / control-only), pairing BWM and control `log2_OR` per (site, amino acid) cell with an `Effect change` (BWM minus control) column; rows grouped A/P/E then sorted by `Effect change` descending. Matches the rendered Olive `.qmd` (step 4 of the within_condition family three-section rollout); tables generated by `scripts/within_condition_sig_split.py`. Underlying significant cells unchanged (8 BWM + 15 control = 23 at FDR<0.05). One number corrected: control P:G `log2_OR` +0.430 -> +0.429 (raw odds_ratio 1.34672, log2 0.42945) across the shared-time-trajectory caveat + For-Chumeng cross-contrast list.
- (readback) "Reconciled shared content from the corrected .qmd on 2026-06-01" → Top hits already mirrored the `.qmd` three-section layout (both / BWM-only / control-only, `effect change` column, no p_value/p_adj columns), so no table change. Every number enumerated and audited: all 19 table cells re-verified against the raw CSV (each BWM/control `log2_OR`, `effect change` = BWM - control, and section membership by FDR<0.05 reconcile); Numbers-at-a-glance, per-(condition, site) counts/min p_adj, and caveat counts all verified. One Dylan-only CSV-column correction: `control-vs-BWM-divergent-direction` body caveat control E:R `p_adj` 0.21 → 0.081 (raw control E,R p_adj = 0.0807; the 0.21 was a stray copy of the adjacent `log2_OR`=-0.21). Cross-file d10_vs_d0 / between_condition_wilcoxon magnitudes in `## For Chumeng` flagged not-CSV-verifiable-here and left as-is. `_INDEX.md` headline row re-synced from the superseded odds_ratio form (BWM↑1.24 etc.) to the current `log2_OR` form to match this file's front-matter headline. Olive-only sections (Biological interpretation, composite + individual plots, Overview) intentionally not imported.
...
Selection rule (updated 2026-05-28): every (site, amino acid) cell significant at FDR<0.05 in at least one condition, split into three sections by which condition(s) reach significance — both, BWM only, control only. Each row pairs the BWM and control `log2_OR` (the within-condition day_10-vs-day_5 Fisher effect: positive = enriched at day_10 relative to day_5, negative = depleted) and reports `Effect change` = BWM `log2_OR` minus control `log2_OR`, a single divergence measure (large magnitude = the two conditions' day_10-vs-day_5 trajectories diverge). Rows are grouped by `site` in A → P → E order, then sorted by `Effect change` descending. Cells significant in neither condition are omitted. Generated by `scripts/within_condition_sig_split.py`.

### Significant in both conditions (n = 4 site x amino acid cells)

| Site | Amino Acid | BWM `log2_OR` | control `log2_OR` | Effect change | Flag |
| --- | --- | --- | --- | --- | --- |
| A | Q (Gln) | -0.309 | -0.534 | +0.225 |  |
| A | R (Arg) | +0.222 | +0.206 | +0.016 |  |
| A | G (Gly) | +0.312 | +0.388 | -0.076 |  |
| P | G (Gly) | +0.297 | +0.429 | -0.133 |  |

### Significant in BWM only (n = 4 site x amino acid cells)

| Site | Amino Acid | BWM `log2_OR` | control `log2_OR` | Effect change | Flag |
| --- | --- | --- | --- | --- | --- |
| A | L (Leu) | -0.399 | +0.028 | -0.427 |  |
| E | R (Arg) | +0.275 | -0.208 | +0.483 |  |
| E | G (Gly) | +0.279 | +0.113 | +0.166 |  |
| E | I (Ile) | -0.268 | +0.115 | -0.383 |  |

### Significant in control only (n = 11 site x amino acid cells)

| Site | Amino Acid | BWM `log2_OR` | control `log2_OR` | Effect change | Flag |
| --- | --- | --- | --- | --- | --- |
| A | N (Asn) | -0.001 | -0.552 | +0.551 |  |
| A | E (Glu) | +0.148 | -0.199 | +0.347 |  |
| A | A (Ala) | -0.179 | +0.267 | -0.446 |  |
| A | P (Pro) | -0.095 | +0.365 | -0.460 |  |
| P | V (Val) | +0.102 | -0.238 | +0.340 |  |
| P | I (Ile) | -0.070 | -0.382 | +0.312 |  |
| P | Q (Gln) | -0.175 | -0.352 | +0.177 |  |
| P | L (Leu) | -0.154 | -0.246 | +0.093 |  |
| P | D (Asp) | +0.066 | +0.199 | -0.132 |  |
| E | A (Ala) | -0.000 | +0.397 | -0.398 |  |
| E | P (Pro) | -0.077 | +0.414 | -0.491 |  |
...
- **large-N-Fisher-anticonservative** *(family-wide)* — per-timepoint per-condition totals are in the thousands (BWM: day_10=6945, day_5=11935; control: day_10=8788, day_5=11177). Tiny effects yield p_adj < 1e-7 (control,P,G `log2_OR`=+0.429 → 5.80e-08). Rank by `log2_OR`. (Inherited.)
...
- **control-vs-BWM-divergent-direction** *(per-CSV)* — three direction-divergent cells: **E:R** (BWM `log2_OR`=+0.275 FDR=0.028 vs control `log2_OR`=-0.21 p_adj=0.081 ns; cleanest flip in the contrast); **A:A** (BWM `log2_OR`=-0.18 ns vs control `log2_OR`=+0.268 FDR=0.017); **P:V** (BWM `log2_OR`=+0.10 ns vs control `log2_OR`=-0.238 FDR=0.021). Several other cells where one condition is sig and the other ns at the same direction (e.g. P:I — both depleted but only control reaches FDR<0.05). The d10_vs_d0 E:K flip does not recur here (both ns) — anchoring that flip at d0.
- **shared-time-trajectory-A/P-Gly** *(per-CSV)* — Both BWM and control enrich Gly at sites A and P from d5→d10 (BWM A:G `log2_OR`=+0.312 p_adj=9.09e-04 / P:G `log2_OR`=+0.297 p_adj=0.00303; control A:G `log2_OR`=+0.388 p_adj=3.96e-06 / P:G `log2_OR`=+0.429 p_adj=5.80e-08 — control,P,G is the file minimum). Coherent shared-time signal across both conditions — not perturbation-specific. Chumeng should treat A/P-Gly enrichment as a time-axis feature, not a BWM-vs-control contrast finding.
...
- Open questions Chumeng should resolve:
  ...
  - **Per-cell observations on site-A/P glycine across contrasts in this family** (cross-contrast magnitudes provided for Chumeng's reconciliation, not as a synthesis claim): d10_vs_d0_aa BWM P:G ns; control A:G `log2_OR`=+0.37, P:G `log2_OR`=+0.36. d10_vs_d5_aa (this file): BWM A:G `log2_OR`=+0.312, P:G `log2_OR`=+0.297, E:G `log2_OR`=+0.279; control A:G `log2_OR`=+0.388, P:G `log2_OR`=+0.429. Does Gly enrichment at sites A and P track with time in both conditions, or is the d5→d10 step the carrier? Cross-check against `between_condition_wilcoxon_aa` (collapsed time, 0 aa hits at FDR<0.05) — would a real both-condition time-axis Gly signal appear there as a near-nominal trend, or would time-pooling cancel it?
