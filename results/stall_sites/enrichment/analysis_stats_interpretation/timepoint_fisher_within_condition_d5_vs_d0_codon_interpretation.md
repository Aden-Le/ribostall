...
user_directives:
  ...
  - "(readback) Reconciled shared content from the corrected .qmd on 2026-06-02 -> NUMBER-AUDIT ONLY (the `.md` three-section Top-hits already mirrored the `.qmd` byte-for-byte; no table reshape). Verified the table identical to the `.qmd` and re-derived all 82 site x codon cells (6 both / 16 BWM-only / 60 control-only) from the raw CSV `odds_ratio` column: BWM/control `log2_OR`, `effect change` = BWM - control, `aa` translation, section placement by FDR<0.05, and low-count flags (codon < 50 in both arms) all clean. Every number enumerated across 3 classes; all scalars (366; 88 = 22+66; 112 = 32+80; min 2.20e-19 control,E,AAG; the 6 per-(condition,site) counts/minima incl. the control,P TAT/AAA 3.40e-07 tie; pooled 18,026 BWM / 38,909 control; zero-cell BWM,E,ATA) and every Dylan-only CSV-column FDR quote verified vs the CSV. **0 corrections to this file.** Cross-file sister/aa-companion magnitudes flagged not-CSV-verifiable."
---
...
- (readback 2026-06-02) Stage-7 NUMBER-AUDIT ONLY pass against the corrected `.qmd`. The three-section Top-hits table already mirrored the `.qmd` byte-for-byte (no reshape). Re-derived all 82 site x codon cells from the raw CSV `odds_ratio` column (BWM/control `log2_OR`, `effect change` = BWM - control, `aa`, FDR<0.05 section placement, low-count `< 50` both-arms flag) — clean; every scalar + every Dylan-only CSV-column FDR quote verified vs the CSV. **0 corrections to this file.** Cross-file sister magnitudes (d10_vs_d0 / d10_vs_d5 codon, aa companion) flagged not-CSV-verifiable. `_INDEX.md` row re-synced from stale odds-ratio framing to the `log2_OR` front-matter headline. Olive-only sections (Biological interpretation, composite + individual plots, Overview) intentionally not imported.
