...
# Fisher CSVs emitted by stall_sites_consensus_intersection_stats.py:
#   between_condition_fisher_{aa,codon}.csv
#
# In the intersection design every group shares one transcript universe, so the
# raw stall-site shares Fisher compares are apples-to-apples. The consensus
# between-condition CSV holds a SINGLE control-vs-treatment comparison and
# therefore has no timepoint/condition grouping column. between_group_volcano.R
...
PLOTS_DIR="./results/c_elegans/stall_sites_consensus_intersection/plots/between_condition_fisher"
