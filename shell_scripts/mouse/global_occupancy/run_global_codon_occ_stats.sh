#!/bin/bash
#----------------------------------------------------
# Bash script: run global_codon_occ_stats.py (MOUSE)
...
# so TIMEPOINTS is left empty below: the between-condition tests (A2 Wilcoxon, A3
# Fisher) run flat, and the between-timepoint tests (A5/A6) are skipped
# automatically.
#----------------------------------------------------

# ============== CONFIG: edit these ==============
# Format: "group1:rep1,rep2;group2:rep1,rep2"
# Global occupancy is per-replicate (NOT consensus-collapsed), so use the actual
# experiment names from the coverage pickle. Mouse: control has 2 reps (AA_3,
# AA_4), treatment 1 (Ch_WAA2).
...
# "day_10" before "day_5"). Mouse has no timepoints, so this is empty and A5/A6
