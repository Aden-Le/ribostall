#!/bin/bash
#----------------------------------------------------
# Shared headline / direction config — MOUSE CONSENSUS UNION stall-sites pipeline.
#
# Sourced by BOTH the stats runner (run_stall_sites_consensus_union_stats.sh,
# which passes the numerator to --headline-condition) AND any analyze_*.sh plot
...
# (delta_log2_enrichment) means "enriched vs background in this condition". Must
# be one of the two condition labels in the runner's EXP_GROUPS (the part before
# the first underscore; for the flat mouse design the whole group label, e.g.
