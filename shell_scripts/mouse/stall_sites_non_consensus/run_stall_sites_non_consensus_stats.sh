#!/bin/bash
#----------------------------------------------------
# Bash script: per-replicate Wilcoxon tests on the stall-site CSVs for MOUSE
...
# so A5 is skipped automatically (TIMEPOINTS is left empty below) and only A2
# runs.
#
...
# Mouse has no timepoints, so this is empty and A5 is skipped automatically.
TIMEPOINTS=''
...
OUT_DIR="./results/mouse/stall_sites_non_consensus/analysis"
...
RUN_BETWEEN_TIMEPOINT_WILCOXON=true     # A5: between-timepoint Wilcoxon (auto-skipped — no timepoints)
...
echo "Timepoints: ${TIMEPOINTS:-<none — A5 skipped>}"
echo "Input: $RAW_DIR"
echo "Output: $OUT_DIR"
