...
# which GENERATES the three cross-group concordance tables (concordant
# enrichment / concordant depletion / discordant) pasted into the matching .qmd
...
# `--sig-threshold 0.10`.
#
# 17/18 blocks GENERATE the three cross-group concordance tables for the
...
# when all 6 groups share one sign, discordant otherwise. Each table shows
# every cell with `#sig` >= `--min-sig` (groups with p_adj<0.05, of 6) -- a
# reproducibility floor on the `#sig` axis, replacing the former top-N rank
# cap; the kept rows are displayed sorted by site A/P/E, then `#sig` desc, then
# `min count` desc. The `log2_enrichment` cell lists the six values on two
# `\newline`-separated lines; the `flag` column aggregates
# iid-amp / bg-tight / rare-aa|rare-codon across the 6 groups. Paste each block
# into the matching Olive .qmd `## Top hits` section (replacing the old
# cross-group concordance pair). Tune with `--min-sig`, `--sig-threshold`,
...
python3 scripts/cross_group_concordance_tables.py "results/stall_sites/enrichment/analysis_stats/within_condition_binomial_aa.csv" --min-sig 2

# ============================================================
# 18. within_condition_binomial_codon  (family: binom_codon)
# ============================================================
echo "---- 18. within_condition_binomial_codon ----"
python3 scripts/cross_group_concordance_tables.py "results/stall_sites/enrichment/analysis_stats/within_condition_binomial_codon.csv" --min-sig 2
